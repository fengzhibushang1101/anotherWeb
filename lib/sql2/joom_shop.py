...
class JoomShop(Base):
    ...
    def raw_upsert(connect, **kwargs):
        ...
        cursor = connect.execute(sql, shop_name=name, shop_no=shop_no, logo=logo, rate=rate, save_count=save_count, create_time=create_time, update_time=update_time, is_verify=is_verify)
        cursor.close()
        return True